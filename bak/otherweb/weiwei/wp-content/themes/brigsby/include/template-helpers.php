...
function hoot_header_aside() {
	...
	?><div id="header-aside" class="header-aside table-cell-mid header-aside-<?php echo $area; ?>"><?php
...
function hoot_update_browser() {
	$notice = '<!--[if lte IE 9]><p class="chromeframe">' .
			  sprintf( __( 'You are using an outdated browser (IE 8 or before). For a better user experience, we recommend %1supgrading your browser today%2s or %3sinstalling Google Chrome Frame%4s', 'brigsby' ), '<a href="http://browsehappy.com/">', '</a>', '<a href="http://www.google.com/chromeframe/?redirect=true">', '</a>' ) .
			  '</p><![endif]-->';
...
function hoot_meta_info_blocks( $args = '', $context = '' ) {
	...
	<div class="entry-byline">
		...
		if ( !empty( $display['author'] ) ) :
			$blocks['author']['label'] = __( 'By:', 'brigsby' );
			ob_start();
		...
		if ( $edit_link = get_edit_post_link() ) :
			...
			$blocks['editlink']['content'] = '<a href="' . $edit_link . '">' . __( 'Edit This', 'brigsby' ) . '</a>';
		endif;
		...
		case 'html':
			...
			for ( $i = 1; $i <= 4;  $i++ ) {
				...
				if ( !empty( $id ) ) {
					...
					$data[$i]['title'] = ( !empty( $postobj->post_title ) ) ? esc_html( $postobj->post_title ) : '';
			...
			for ( $i = 1; $i <= 4;  $i++ ) {
				...
				$slides[ $i ]['content'] = '<h4>' . $data[$i]['title'] . '</h4>' . $data[$i]['content'];
